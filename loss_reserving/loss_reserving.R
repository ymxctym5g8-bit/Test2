...
#
# Nutzung als Bibliothek:  source("loss_reserving.R"); r <- compute_all(tri, ...)
#
# Eingabe: CSV mit erster Spalte Anfalljahr, weiteren Spalten Abwicklungsjahre,
#          zukuenftige Zellen leer. Praemien-CSV: Anfalljahr, Praemie.
# Pakete:  nur Base R; optional openxlsx (Excel-Export) und readxl (Excel-Import).
# =============================================================================
...
detect_sep <- function(line) {
  cand <- c(";" = ";", "\t" = "\t", "," = ",")
  counts <- vapply(cand, function(s) lengths(regmatches(line, gregexpr(s, line, fixed = TRUE))), 0L)
  if (all(counts == 0)) return(",")
  names(cand)[which.max(counts)]
}

# Tabelle (data.frame, erste Spalte = Anfalljahr) -> Dreieck
triangle_from_frame <- function(df, incremental = FALSE, decimal = ".") {
  df <- as.data.frame(df, check.names = FALSE, stringsAsFactors = FALSE)
  origins <- trimws(as.character(df[[1]]))
  vals <- as.matrix(as.data.frame(lapply(df[-1], function(col)
    if (is.numeric(col)) col else parse_num(col, decimal)), check.names = FALSE))
...
read_triangle_text <- function(text, incremental = FALSE, sep = NULL, decimal = ".") {
  lines <- strsplit(trimws(text), "\r?\n")[[1]]
  lines <- lines[nzchar(trimws(lines))]
  if (is.null(sep) || sep == "auto") sep <- detect_sep(lines[1])
  df <- utils::read.table(text = lines, sep = sep, header = TRUE, colClasses = "character",
                          check.names = FALSE, na.strings = c("", "NA"), quote = "\"",
                          comment.char = "", fill = TRUE, strip.white = TRUE)
  triangle_from_frame(df, incremental, decimal)
}

read_triangle_file <- function(path, incremental = FALSE, sep = NULL, decimal = ".", name = path) {
  if (grepl("\\.xlsx?$", tolower(name))) {
    if (!requireNamespace("readxl", quietly = TRUE)) stop("F\u00fcr Excel-Dateien wird das Paket 'readxl' ben\u00f6tigt.")
    return(triangle_from_frame(as.data.frame(readxl::read_excel(path)), incremental, decimal))
  }
  read_triangle_text(paste(readLines(path, warn = FALSE, encoding = "UTF-8"), collapse = "\n"),
                     incremental, sep, decimal)
}

read_vector_file <- function(path, tri, sep = NULL, decimal = ".", what = "Pr\u00e4mien", positive = TRUE) {
  lines <- readLines(path, warn = FALSE, encoding = "UTF-8")
  lines <- lines[nzchar(trimws(lines))]
  if (is.null(sep) || sep == "auto") sep <- detect_sep(lines[1])
  df <- utils::read.table(text = lines, sep = sep, header = TRUE, colClasses = "character",
                          check.names = FALSE)
  p <- parse_num(df[[2]], decimal)
  names(p) <- trimws(df[[1]])
  if (tri$period == "year" && length(p) && all(vapply(names(p), function(o) !is.null(parse_quarter(o)), TRUE))) {
    y <- vapply(names(p), function(o) parse_quarter(o)[1], 0L)
    p <- tapply(p, as.character(y), sum)                    # Quartalswerte -> Jahre
  }
  if (all(tri$origins %in% names(p))) p <- p[tri$origins]
  else if (length(p) == tri$I) names(p) <- tri$origins
  else stop(what, " passen weder nach Anfalljahr noch nach Anzahl zum Dreieck.")
  if (anyNA(p)) stop(what, ": f\u00fcr jedes Anfalljahr wird ein Wert ben\u00f6tigt.")
  if (positive && any(p <= 0)) stop(what, " m\u00fcssen f\u00fcr alle Anfalljahre positiv sein.")
  unname(p)
}
read_premiums_file <- function(path, tri, sep = NULL, decimal = ".")
  read_vector_file(path, tri, sep, decimal, "Pr\u00e4mien", positive = TRUE)
read_benchmark_file <- function(path, tri, sep = NULL, decimal = ".")
  read_vector_file(path, tri, sep, decimal, "Manuelle Reserve", positive = FALSE)
...
demo_quarterly <- function(to_years = FALSE, dir = DATA_DIR) {
  tri <- read_triangle_file(file.path(dir, "beispiel_quartale.csv"), sep = ";")
  pr <- utils::read.table(file.path(dir, "beispiel_quartale_praemien.csv"), sep = ";", header = TRUE,
                          colClasses = c("character", "numeric"))
  p <- setNames(pr[[2]], trimws(pr[[1]]))
  prep <- prepare_triangle(tri, "quarter", to_years)
  if (isTRUE(to_years)) {
    y <- vapply(names(p), function(o) parse_quarter(o)[1], 0L)
    p <- tapply(p, as.character(y), sum)
  }
  list(tri = prep$tri, premiums = unname(p[prep$tri$origins]), notes = prep$notes)
}
...
parse_args <- function(args) {
  opt <- list(triangle = NULL, premiums = NULL, benchmark = NULL, period = "auto", `to-years` = FALSE, elr = NULL, tail = 1,
              `n-sims` = 5000, seed = 42, sep = "auto", decimal = ".", incremental = FALSE,
              `output-dir` = "reserving_output")
  i <- 1
  while (i <= length(args)) {
    key <- sub("^--", "", args[i])
...
main <- function(args = commandArgs(trailingOnly = TRUE)) {
  ...
  opt <- parse_args(args)
  if (isTRUE(opt$help)) {
    cat("Rscript loss_reserving.R [--triangle datei.csv] [--premiums praemien.csv] [--elr 0.65]\n",
        "  [--benchmark manuell.csv] [--period auto|year|quarter] [--to-years] [--tail 1.0] [--n-sims 5000] [--seed 42] [--sep ';'] [--decimal ',']\n",
        "  [--incremental] [--output-dir reserving_output]\n")
    return(invisible())
  }
  if (!is.null(opt$triangle)) {
    tri <- read_triangle_file(opt$triangle, opt$incremental, opt$sep, opt$decimal)
    prep <- prepare_triangle(tri, opt$period, opt$`to-years`)
    tri <- prep$tri
    for (n in prep$notes) cat("Hinweis:", n, "\n")
    premiums <- if (!is.null(opt$premiums)) read_premiums_file(opt$premiums, tri, opt$sep, opt$decimal)
    cat(sprintf("Dreieck geladen: %s (%d %s x %d %s)\n", opt$triangle, tri$I, tri$labels$origin_pl, tri$J, tri$labels$dev_pl))
  } else {
  ...
  benchmark <- if (!is.null(opt$benchmark)) read_benchmark_file(opt$benchmark, tri, opt$sep, opt$decimal)
